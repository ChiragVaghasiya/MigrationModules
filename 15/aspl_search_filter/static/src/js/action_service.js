...
function makeActionManager(env) {
    ...
    async function  _executeActWindowAction(action, options) {
        ...
        if (viewDescriptions.search  && viewDescriptions.search.irFilters && viewDescriptions.search.irFilters.length > 0){
            const view = viewDescriptions.search.irFilters
            var defaultFavoriteSearch = view.find(search => search.is_default);
            if (defaultFavoriteSearch){
                var contextObject = JSON.parse(defaultFavoriteSearch.context.replace(/'/g, "\""));
                var loadViewValue = contextObject.load_view;
            }
        }

        let view = ''
        if (defaultFavoriteSearch){
            view = loadViewValue && views.find((v) => v.type === loadViewValue);
        }else{
            view = options.viewType && views.find((v) => v.type === options.viewType);
        }

        // let view = 'kanban' && views.find((v) => v.type === 'kanban');
