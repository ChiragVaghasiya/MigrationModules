...
class ItDeclarationPayslip(models.Model):
    ...
    # Sec 80C
    five_year_fixed_deposite = fields.Integer(string="80C Five Years of Fixed Deposite in Scheduled Bank",
                                              tracking=True)
    ...
    @api.onchange('five_year_fixed_deposite', 'children_tution_fees', 'contribution_to_pension_fund', 'deposite_in_nsc',
    ...
    def calculate_it_declaration(self):
        ...
        if active_employee_data:
            for emp_rec in active_employee_data:
                it_declation_id = self.env['it.declaration.payslip'].search(
                    [('employee_id', '=', emp_rec.id), ('financial_year', '=', financial_year.id)])
...
class HomerentAllowance(models.TransientModel):
...
class IncomeLoss(models.TransientModel):
    _name = 'income.loss'
...
class OtherIncome(models.TransientModel):
    _name = 'other.income'
