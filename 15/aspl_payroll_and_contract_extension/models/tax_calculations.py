...
class FinancialYear(models.Model):
    ...
    name = fields.Char(string="Fiancial Year")
...
class tax_calculations(models.Model):
    ...
    financial_year = fields.Many2one('financial.year',string="Financial Year",tracking=True)
    ...
    def onchange_function_deduction(self):
        ...
        # total_declared_vi_a_deductions = 0
        # if self.total_declared_vi_a_deductions > 75000:
        #     total_declared_vi_a_deductions = 75000
        # else:
        #     total_declared_vi_a_deductions = self.total_declared_vi_a_deductions
    ...
    @api.onchange('applicable_declared_80c','applicable_declared_vi_a_deductions','total_declared_hra','applicable_declared_medical','total_exemption','total_declared_other','total_tax_previous_employer')
    ...
    def calculate_it_declaration(self):
        active_employee_data = self.env['hr.employee'].search([])

        current_financial_year = str(date.today().strftime("%Y")) + '-' + str(int(date.today().strftime("%y"))+1)
        financial_year = self.env['financial.year'].search([('name','=',current_financial_year)])
        if not financial_year:
            financial_year =self.env['financial.year'].create({'name':current_financial_year})

        if active_employee_data:
            for emp_rec in active_employee_data:
                it_declation_id = self.env['it.declaration.payslip'].search([('employee_id', '=', emp_rec.id),('financial_year','=',financial_year.id)])
                if not it_declation_id:
                    self.env['it.declaration.payslip'].create({
    ...
    def grand_amount(self):
        ...
        _logger.info("Declaration values are: %s", grand_amount)
        return grand_amount
    ...
    def write(self, vals):
        _logger.info("VALS: %s   Status:%s",vals,self.status)
        is_editable = True
        computed_fields = ['total_declared_80c','grand_total','applicable_declared_80c','applicable_declared_vi_a_deductions','applicable_declared_medical','status']
        for key in  vals:
            is_editable = True if key in computed_fields or self.status=='unlocked' else False
            _logger.info("Key: %s Editable:%s ",key,is_editable)
            _logger.info("declaration status match:%s", self.status == 'unlocked')
        if is_editable:
            super(tax_calculations,self).write(vals)
        else:
            raise ValidationError('IT declaration can only be updated when it is unlocked. Please contact accountant in case you want to make some changes.')
...
class Hra(models.TransientModel):
    ...
    @api.onchange('monthly_rent_amount')
    def depends_function_annual_rent(self):
        for rent in self:
            if rent.from_possession:
                month_start = datetime.strptime(str(rent.from_possession), "%Y-%m-%d").strftime('%m')
                month_end = datetime.strptime(str(rent.to_possession), "%Y-%m-%d").strftime('%m')
                rent.annual_rent_amount = rent.monthly_rent_amount*((int(month_end)-int(month_start))+1)
            else: 
