...
companyService.start = function(env, { user, router, cookie }) {
       let cids;
        ...
        if (session.is_login){
            cids = Object.keys(session.user_companies.allowed_companies).map(Number);
        }
        ...
        return {
            ...
            set allowedCompanyIds(cids) {
                allowedCompanyIds = cids;
            },
            get currentCompany() {
                debugger
                if (session.is_login){
                    return availableCompanies[session.user_companies.current_company]
                }
